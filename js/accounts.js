...
async function registerAccount(username, password, role) {
  /* 优先 API */
  if (API.getToken()) {
    try { await API.createUser(username, password, role); return true; }
    catch (e) { /* 降级到本地 */ }
  }
...
async function loginAccount(username, password) {
  ...
  if (API.BASE) {
    try {
      ...
      /* API 失败，降级到本地 IndexedDB */
      if (e.message !== 'Failed to fetch') throw e;  // 非网络错误直接抛
    }
  }
  /* 本地 IndexedDB 登录（离线兜底） */
  return new Promise(function(ok, no) {
    var d = getBBDB() || db;
    if (!d) { no('数据库未就绪'); return; }
    try {
...
function logoutAccount() {
  ...
  ['gh_token','llm_key','dispatch_user','shipxy_key','dispatch_jwt'].forEach(function(k) {
...
async function openAccountModal() {
  ...
  /* 优先 API */
  if (API.getToken()) {
    try { accounts = await API.listUsers(); }
    catch (e) { /* 降级 */ }
  }
  /* 本地兜底 */
  if (!accounts.length) {
    var d = getBBDB() || db;
    try {
      accounts = await new Promise(function(ok) {
        var tx = d.transaction('accounts', 'readonly');
        var st = tx.objectStore('accounts');
        var r = st.getAll();
...
async function changeRole(username, newRole) {
  ...
  if (API.getToken()) {
    try { await API.updateUserRole(username, newRole); openAccountModal(); return; }
    catch (e) { /* 降级到本地 */ }
  }
  var d = getBBDB() || db;
  try {
...
async function resetPassword(username) {
  ...
  if (API.getToken()) {
    try { await API.resetUserPassword(username, newPwd); alert('✅ ' + username + ' 密码已重置'); openAccountModal(); return; }
    catch (e) { /* 降级到本地 */ }
  }
  var d = getBBDB() || db;
  try {
...
async function deleteAccount(username) {
  ...
  if (API.getToken()) {
    try { await API.deleteUser(username); openAccountModal(); return; }
    catch (e) { /* 降级到本地 */ }
  }
  var d = getBBDB() || db;
  try {
